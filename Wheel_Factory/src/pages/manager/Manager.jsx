...
const ManagerDashboard = () => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        toast({
          ...
          description: "There was a problem retrieving the orders. Please try again.",
          variant: "destructive",
          duration: 5000,
        });
      }
  ...
  const fetchOrderDetails = async (orderId) => {
    try {
      ...
      toast({
        ...
        description: "There was a problem retrieving the order details. Please try again.",
        variant: "destructive",
        duration: 5000,
      });
    }
  ...
  const fetchScrappedDetails = async (orderId) => {
    try {
      ...
      toast({
        ...
        description: "There was a problem retrieving the scrapped order details. Please try again.",
        variant: "destructive",
        duration: 5000,
      });
    }
  ...
  const fetchCompletedDetails = async (orderId) => {
    try {
      ...
      const taskResponse = await axios.get(FirstEndpoint, { headers: { Authorization: `Bearer ${token}` } });
      ...
      try {
        if (secondEndpoint) {
          const additionalResponse = await axios.get(secondEndpoint, { headers: { Authorization: `Bearer ${token}` } });
          additionalData = additionalResponse.data;
        }
        if (ThirdEndpoint) {
          const SecondResponse = await axios.get(ThirdEndpoint, { headers: { Authorization: `Bearer ${token}` } });
          additionalsecond = SecondResponse.data;
        }
        if (FourthEndpoint) {
          const ThirdResponse = await axios.get(FourthEndpoint, { headers: { Authorization: `Bearer ${token}` } });
      ...
      toast({
        ...
        description: "There was a problem retrieving the completed order details. Please try again.",
        variant: "destructive",
        duration: 5000,
      });
    }
          ...
          {isModalOpen && selectedOrder && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white p-4 rounded-lg shadow-xl w-1/2 max-w-2xl relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-serif font-semibold">
                ...
                <div className="font-serif font-semibold text-black">
                  ...
                  {selectedOrder.imageUrl && (
                    <img src={selectedOrder.imageUrl} alt="Order" className="max-w-full h-32 object-contain mt-2" />
                  )}
                </div>
          ...
          {isModalOpen && viewCompletedDetails && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-teal-500 p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
                ...
                <h2 className="text-2xl font-bold mb-4 font-sans text-center">Completed Order Details</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Left Column: Order Details and Sand Blasting Details */}
                  <div className="mt-2">
                    {/* Order Details Card */}
                    <div className="bg-white p-4 rounded-lg shadow-md mb-4">
                      <h3 className="text-xl font-semibold text-center font-sans">Order Details</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="font-sans">
                          ...
                          {viewCompletedDetails.imageUrl && (
                            <div className="mt-2">
                              ...
                              {showImage && (
                                <img src={viewCompletedDetails.imageUrl} alt="Order" className="mt-2 max-w-full h-32 object-contain" />
                              )}
                            </div>
                          )}
                        </div>
                    ...
                    {viewCompletedDetails.additionalData && (
                      <div className="bg-white p-4 rounded-lg shadow-md">
                        <h3 className="text-xl font-semibold text-center font-sans">Sand Blasting Details</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="font-medium font-sans">
                            <p><strong>Notes:</strong></p>
                          ...
                          <div className="font-sans">
                            <p>{viewCompletedDetails.additionalData[0].notes}</p>
                            <p>{viewCompletedDetails.additionalData[0].sandBlastingLevel}</p>
                            <button
                              className="mt-2 px-2 py-1 bg-red-400 text-black rounded hover:bg-gray-400 transition-colors"
                              onClick={() => setShowSandBlastingImage(prev => !prev)} // Toggle image visibility
                            >
                              {showSandBlastingImage ? "Hide Image" : "Show Image"}
                            </button>
                            {showSandBlastingImage && viewCompletedDetails.additionalData[0].imageUrl && (
                              <img src={viewCompletedDetails.additionalData[0].imageUrl} alt="Sand Blasting" className="mt-2 max-w-full h-32 object-contain" />
                            )}
                          </div>
                        </div>
                      </div>
                  ...
                  <div>
                    ...
                    {viewCompletedDetails.additionalsecond && (
                      <div className="bg-white p-4 rounded-lg shadow-md mb-4">
                        <h3 className="text-xl font-semibold text-center font-sans">Painting Details</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="font-medium font-sans">
                            <p><strong>Notes:</strong></p>
                          ...
                          <div className="font-sans">
                            <p>{viewCompletedDetails.additionalsecond[0].notes}</p>
                            <p>{viewCompletedDetails.additionalsecond[0].pColor}</p>
                            <p>{viewCompletedDetails.additionalsecond[0].pType}</p>
                            <button
                              className="mt-2 px-2 py-1 bg-red-400 text-black rounded hover:bg-gray-400 transition-colors"
                              onClick={() => setShowPaintingImage(prev => !prev)} // Toggle image visibility
                            >
                              {showPaintingImage ? "Hide Image" : "Show Image"}
                            </button>
                            {showPaintingImage && viewCompletedDetails.additionalsecond[0].imageUrl && (
                              <img src={viewCompletedDetails.additionalsecond[0].imageUrl} alt="Painting" className="mt-2 max-w-full h-32 object-contain" />
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    {viewCompletedDetails.additionalthird && (
                      <div className="bg-white p-4 rounded-lg shadow-md">
                        <h3 className="text-xl font-semibold text-center font-sans">Packaging Details</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="font-medium font-sans">
                            <p><strong>Notes:</strong></p>
                          ...
                          <div className="font-sans">
                            <p>{viewCompletedDetails.additionalthird[0].notes}</p>
                            <p>{viewCompletedDetails.additionalthird[0].iRating}</p>
                            <button
                              className="mt-2 px-2 py-1 bg-red-400 text-black rounded hover:bg-gray-400 transition-colors"
                              onClick={() => setShowPackagingImage(prev => !prev)} // Toggle image visibility
                            >
                              {showPackagingImage ? "Hide Image" : "Show Image"}
                            </button>
                            {showPackagingImage && viewCompletedDetails.additionalthird[0].imageUrl && (
                              <img src={viewCompletedDetails.additionalthird[0].imageUrl} alt="Packaging" className="mt-2 max-w-full h-32 object-contain" />
                            )}
                          </div>
                        </div>
                      </div>
              ...
              <div className="bg-gray-300 p-6 rounded-lg shadow-xl w-full max-w-3xl">
                ...
                <form onSubmit={formik.handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col">
                    <div>
                      <label htmlFor="clientName" className="block text-m font-medium text-gray-900">Client Name</label>
                      <input
                        id="clientName"
                        name="clientName"
                        type="text"
                    ...
                    <div>
                      <label htmlFor="year" className="block text-m font-medium text-gray-900 mt-4">Year</label>
                      <input
                        id="year"
                        name="year"
                        type="text"
                    ...
                    <div>
                      <label htmlFor="make" className="block text-m font-medium text-gray-900 mt-4">Make</label>
                      <input
                        id="make"
                        name="make"
                        type="text"
                      ...
                      {formik.touched.make && formik.errors.make ? (
                        <div className="text-red-500 text-sm mt-1">{formik.errors.make}</div>
                      ) : null}
                    </div>
                    <div>
                      <label htmlFor="notes" className="block text-m font-medium text-gray-900 mt-4">Notes</label>
                      <textarea
                        id="notes"
                        name="notes"
                        onChange={formik.handleChange}
                  ...
                  <div className="flex flex-col">
                    <div>
                      <label htmlFor="model" className="block text-m font-medium text-gray-900">Model</label>
                      <input
                        id="model"
                        name="model"
                        type="text"
                    ...
                    <div>
                      <label htmlFor="damageType" className="block text-m font-medium text-gray-900 mt-4">Damage Type</label>
                      <input
                        id="damageType"
                        name="damageType"
                        onChange={formik.handleChange}
                      ...
                      {formik.touched.damageType && formik.errors.damageType ? (
                        <div className="text-red-500 text-sm mt-1">{formik.errors.damageType}</div>
                      ) : null}
                    </div>
                    <div>
                      <label htmlFor="status" className="block text-m font-medium text-gray-900 mt-4">Status</label>
                      <input
                        id="status"
                        name="status"
                        type="text"
                    ...
                    <div>
                      <label htmlFor="imageUrl" className="block text-m font-medium text-gray-900 mt-4">Image</label>
                      <input
                        id="imageUrl"
                        name="imageUrl"
                        type="file"
...
const ScrappedOrdersTable = ({
  ...
  if (!scrappedOrders || scrappedOrders.length === 0) {
    return <p className="text-gray-500 text-center py-4">No scrapped orders available.</p>;
  }
